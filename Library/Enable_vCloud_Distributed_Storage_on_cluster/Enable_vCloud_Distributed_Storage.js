/**
 * Enable vCloud Distributed Storage
 *
...
if (!isEnabled) {
	...
	clusterConfigSpec.vsanConfig.enabled = true;
	...
	System.log("Turning on vCloud Distributed Storage for " + cluster.name);

	task = cluster.reconfigureComputeResource_Task(clusterConfigSpec, true);
}
else {
	System.log("vCloud Distributed Storage is already enabled for " + cluster.name);
}