...
if (result == null)
{
    throw "Network container is either full or does not exist"
}
var subnet = extractSubnet(networkString);
network = subnet.split("/")[0];
if (cidr <= 29)
{
const range = calculateIpRange(subnet, cidr);
startAddress = range.firstUsableIp;
endAddress = range.lastUsableIp;
name = "range for gateway";
}
else 
{
    throw "cannot reserve range for a smaller network than /30"
}
...
function calculateIpRange(subnet, cidr) {
    ...
    var lastUsableLong = networkLong + 1;
