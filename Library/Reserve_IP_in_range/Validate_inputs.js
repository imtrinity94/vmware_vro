...
if (startAddress) {
	var validationError = System.getModule("com.infoblox.ipam.util").validateIPAddress(startAddress);
	if (validationError) {
		throw validationError;
	}
} else {
	throw "Start address of the network range is not specified";
}

if (endAddress) {
	var validationError = System.getModule("com.infoblox.ipam.util").validateIPAddress(endAddress);
	if (validationError) {
		throw validationError;
	}
} else {
	throw "End address of the network range is not specified";
}

if (createReservation) {
	var isIPv4StartAddress = System.getModule("com.infoblox.ipam.util").isIPv4Address(startAddress);
	var isIPv4EndAddress = System.getModule("com.infoblox.ipam.util").isIPv4Address(endAddress);
	if (!isIPv4StartAddress || !isIPv4EndAddress) {
