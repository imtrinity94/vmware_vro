 ...
 * @param {StoreServ:Volume} volumeToMount
 */
var lc = volumeToMount.wwn;
lc = lc.toLowerCase();
var devNaa = ("naa." + lc);
...
for (var i=0; i < hostsToRescan.length; i++) {
	...
	if(!foundVolume)
	{
		var volAttachError = "Error while detaching volume. Volume: " + diskCanonicalName + " not found on the ESXi host: " + hostObj.name;
...
for each (var dstore in allDS){	
	if (dstore != undefined) {
		if (dstore.info != undefined) {
			if (dstore.info.vmfs != undefined) {
